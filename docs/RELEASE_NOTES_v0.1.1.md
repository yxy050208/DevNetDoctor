# DevNet Doctor v0.1.1 — Windows 开发工具代理诊断
...
DevNet Doctor 将系统代理、环境变量、端口监听状态和网络路径测试放在一起，帮助定位问题来源。v0.1.1 重点支持 Codex 和 CCSwitch 的诊断。
...
- 生成脱敏诊断报告，方便求助。
- 分别测试直连 IPv4 和 IPv6，记录 HTTP 版本、对端地址和 TLS 证书摘要。
- 使用不带登录数据的 `HEAD` 请求检查 device-auth 端点的传输层响应。
- 当证书校验失败或不同路径出现不同证书签发者时给出谨慎提示。

## 下载哪个文件？

- **DevNetDoctor.exe**：Windows x64 图形界面版，自带 .NET 运行时，下载后运行。
- **DevNetDoctor-v0.1.1-source.zip**：源码包，含便携 PowerShell 扫描器 `DevNetDoctor.ps1`。
...
本版不会自动关闭安全软件、跳过证书校验或声称已确认 Codex 版本回归。网络探测成功只表示本工具的 Windows/.NET 请求收到预期响应，不能代表 Codex、Desktop 或 WSL 的 token exchange 一定成功。
